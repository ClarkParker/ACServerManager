...
angular.module('acServerManager')
	.controller('StatusCtrl', function($scope, $timeout, ProcessService, ServerService) {
		...
		(function getMinoRatingServerStatus() {
			ProcessService.MinoRatingServerStatus(function(data){
				$scope.minoRatingServerStatus = data.status;
				$timeout(getMinoRatingServerStatus, 2000);
			});
		})();
		...
		$scope.startMinoRatingServer = function() {
			ProcessService.StartMinoRatingServer(function(result) {
				if (!(result[0] === 'O' && result[1] === 'K')) {
					createAlert('warning', 'Failed to start MinoRating', true)
				}
			})
		}
		...
		$scope.stopMinoRatingServer = function() {
			ProcessService.StopMinoRatingServer(function(result) {
				if (!(result[0] === 'O' && result[1] === 'K')) {
					createAlert('warning', 'Failed to stop MinoRating', true)
				}
			})
		}
		...
		$scope.restartMinoRatingServer = function() {
			ProcessService.RestartMinoRatingServer(function(result) {
				if (!(result[0] === 'O' && result[1] === 'K')) {
					createAlert('warning', 'Failed to restart MinoRating', true)
				}
			})
		}
